...
//  console.log(serviceId, address, date);
    ...
    if (bookingDate <= today) {
      return NextResponse.json({ message: "Booking date must be later than today", success: false }, { status: 400 });
    }
...
const booking = new Booking({
  ...
  createdAt: new Date(),
    ...
    return NextResponse.json({ message: "Booking successful", success: true }, { status: 200 });

  } catch (error) {
    console.error("Booking API error:", error);
    return NextResponse.json({ message: "Booking failed", success: false }, { status: 500 });
  }
}
