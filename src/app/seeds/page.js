"use client";

import { useEffect, useState } from "react";
import { useDispatch, useSelector } from "react-redux";
import Link from "next/link";
import { useRouter } from "next/navigation";
import { FaCartPlus } from "react-icons/fa";
import { addToCart } from "../../redux/store/cardSlice";
import styles from "../plants/plants.module.css";

export default function SeedsPage() {
  const [seeds, setSeeds] = useState([]);
  const [filteredSeeds, setFilteredSeeds] = useState([]);
  const [searchQuery, setSearchQuery] = useState("");
  const dispatch = useDispatch();
  const router = useRouter();
  const isAdmin = useSelector((state) => state.admin.isAdmin);

  useEffect(() => {
    const fetchSeeds = async () => {
      try {
        const res = await fetch("/api/seeds");
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        const data = await res.json();
        setSeeds(data);
        setFilteredSeeds(data);
      } catch (err) {
        console.error("Error fetching seeds:", err);
      }
    };

    fetchSeeds();
  }, []);

  useEffect(() => {
    const query = searchQuery.toLowerCase();
    const filtered = seeds.filter(
      (seed) =>
        seed.name.toLowerCase().includes(query) ||
        seed.category.toLowerCase().includes(query) ||
        seed.pricePerUnit.toString().includes(query)
    );
    setFilteredSeeds(filtered);
  }, [searchQuery, seeds]);

  const handleAddToCart = (seed) => {
    dispatch(addToCart(seed));
  };

  return (
    <div className="min-h-screen px-6 pt-15 bg-white pb-4">
      <div className="max-w-6xl mx-auto text-center">
        <h1 className="text-3xl font-bold text-green-700 mb-6">Available Seeds</h1>

        {isAdmin && (
          <Link href="/seeds/add">
            <button className="mb-4 px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700 cursor-pointer">
              Add New Seed
            </button>
          </Link>
        )}

        {/* Search Bar with Clear */}
        <div className="relative w-full px-5 mx-auto mb-4">
          <input
            type="text"
            placeholder="Search by name or category..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="w-full px-4 py-2 pr-10 border rounded-md"
          />
          {searchQuery && (
            <button
              onClick={() => setSearchQuery("")}
              className="cursor-pointer absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 hover:text-black text-xl font-semibold pr-3"
              aria-label="Clear"
            >
              &times;
            </button>
          )}
        </div>

        {/* Category Filter Buttons */}
        <div className="flex flex-wrap gap-2 justify-center mb-6">
          {["flower", "vegetable", "herb", "fruit"].map((label) => (
            <button
              key={label}
              onClick={() => setSearchQuery(label)}
              className="cursor-pointer px-3 py-1 bg-green-100 text-green-800 rounded-full hover:bg-green-200 text-sm"
            >
              {label.charAt(0).toUpperCase() + label.slice(1)}
            </button>
          ))}
        </div>

        {/* Seeds Grid */}
        <div
          className={`grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-5 gap-6 justify-items-center ${styles.plantGrid}`}
        >
          {filteredSeeds.length === 0 ? (
            <p className="col-span-full text-gray-600">No matching seeds found.</p>
          ) : (
            filteredSeeds.map((seed) => (
              <div
                key={seed._id}
                className="bg-white rounded-lg shadow-md flex flex-col"
              >
                <img
                  src={seed.imageUrl}
                  alt={seed.name}
                  className="w-45 h-35 sm:h-40 object-fill rounded-lg"
                />
                <h2 className="text-xl font-semibold text-green-800 mb-1">
                  {seed.name}
                </h2>
                <div className="flex justify-around items-center mb-2">
                  <span className="text-green-700 font-bold text-lg">
                    ₹{seed.pricePerUnit}
                    <span className="text-sm text-gray-600"> ({seed.priceUnit})</span>
                  </span>
                </div>
                <div className="grid grid-cols-[2fr_1fr] gap-1 mt-auto">
                  <Link href={`/seeds/${seed._id}`} className="w-full">
                    <div className="bg-blue-500 text-white text-center w-full px-1 py-1.5 rounded hover:bg-blue-600 text-sm">
                      View Product
                    </div>
                  </Link>
                  <button
                    onClick={() => handleAddToCart(seed)}
                    className={`cursor-pointer px-1 py-1.5 rounded text-sm flex items-center justify-center w-full ${
                      seed.inStock
                        ? "bg-green-500 text-white hover:bg-green-600"
                        : "bg-gray-400 text-gray-700 cursor-not-allowed"
                    }`}
                    title={seed.inStock ? "Add to Cart" : "Out of Stock"}
                    disabled={!seed.inStock}
                  >
                    <FaCartPlus className="text-lg" />
                  </button>
                </div>
              </div>
            ))
          )}
        </div>
      </div>
    </div>
  );
}
